...
export function SocialShare({
  title = 'Check out my Bas 5 Minute map!',
  text = 'I created this map showing how far you can travel in 5 minutes. Is it actually 5 minutes though? 🤔',
  url,
  className = '',
}: SocialShareProps) {
  ...
  return (
    <div className={`space-y-4 ${className}`}>
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-bold text-gray-900">Share Your Map</h3>
        {hasNativeShare && (
          <button
            onClick={nativeShare}
            className="px-4 py-2 text-sm font-medium bg-gradient-to-r from-pink-600 to-orange-500 text-white rounded-lg hover:shadow-lg transition"
          >
            📤 Share
          </button>
        )}
      ...
      <div className="grid grid-cols-2 gap-3">
        {/* Twitter/X */}
        <button
          onClick={() => handleShare('twitter', () => window.open(shareLinks.twitter, '_blank'))}
        ...
        {/* Facebook */}
        <button
          onClick={() => handleShare('facebook', () => window.open(shareLinks.facebook, '_blank'))}
        ...
        {/* WhatsApp */}
        <button
          onClick={() => handleShare('whatsapp', () => window.open(shareLinks.whatsapp, '_blank'))}
        ...
        {/* LinkedIn */}
        <button
          onClick={() => handleShare('linkedin', () => window.open(shareLinks.linkedin, '_blank'))}
        ...
        {/* Reddit */}
        <button
          onClick={() => handleShare('reddit', () => window.open(shareLinks.reddit, '_blank'))}
          className="flex items-center justify-center gap-2 px-4 py-3 bg-orange-600 text-white rounded-lg hover:bg-orange-700 transition font-medium"
        >
          <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
      ...
      <p className="text-xs text-gray-500 text-center">
        Share your creation and spread the traffic humor! 🚗💨
      </p>
    </div>
  );
}
